...
from qudi.interface.excitation_scanner_interface import (
    ExcitationScannerInterface,
    ExcitationScannerConstraints,
    ExcitationScanControlVariable,
    ExcitationScanDataFormat,
)
from qudi.interface.sampled_finite_state_interface import (
...
class FiniteSamplingScanningExcitationInterfuse(
    ExcitationScannerInterface, SampledFiniteStateInterface
):
    """
    ...
    _finite_sampling_io = Connector(
        name="scan_hardware", interface="FiniteSamplingIOInterface"
    )
    _ao = Connector(name="analog_output", interface="ProcessSetpointInterface")
    _ext_process = Connector(
        name="external_process", interface="ProcessValueInterface", optional=True
    )
    _ext_setpoint = Connector(
        name="external_setpoint", interface="ProcessSetpointInterface", optional=True
    )
    ...
    _sleep_time_external_stabilization = StatusVar(
        name="sleep_time_external_stabilization", default=60
    )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._scanning_states = {
            "prepare_scan",
            "prepare_step",
            "wait_ready",
            "record_scan_step",
    ...
    def on_activate(self):
        ...
        variables = [
            ExcitationScanControlVariable(
                "Conversion factor", (0.0, 1e11), float, "Hz/V"
            ),
            ExcitationScanControlVariable(
                "Minimum tension",
                (self._minimum_tension, self._maximum_tension),
                float,
                "V",
            ),
            ExcitationScanControlVariable(
            ...
            ExcitationScanControlVariable(
                "Tension step",
                (
                    self._minimum_tension_step,
                    self._maximum_tension - self._minimum_tension,
                ),
                float,
                "V",
            ),
            ExcitationScanControlVariable(
                "Minimum frequency", (-10e12, 10e12), float, "Hz"
            ),
            ExcitationScanControlVariable(
                "Maximum frequency", (-10e12, 10e12), float, "Hz"
            ),
            ...
            ExcitationScanControlVariable(
                "Idle frequency", (-10e12, 10e12), float, "Hz"
            ),
            ExcitationScanControlVariable("External enabled", (False, True), bool, ""),
        ]
        if self._ext_setpoint() is not None:
            limits = self._ext_setpoint().constraints.channel_limits[
                self._setpoint_channel
            ]
            variables.append(
                ExcitationScanControlVariable(
                    "First external setpoint",
                    limits,
                    float,
                    self._ext_setpoint().constraints.channel_units[
                        self._setpoint_channel
                    ],
                ),
            )
            ...
            variables.append(
                ExcitationScanControlVariable(
                    "External setpoint step",
                    (0, abs(limits[0] - limits[1])),
                    float,
                    self._ext_setpoint().constraints.channel_units[
                        self._setpoint_channel
                    ],
            ...
            variables.append(
                ExcitationScanControlVariable(
                    "External setpoint repeat per step", (1, 100), int, ""
                )
            )
            variables.append(
                ExcitationScanControlVariable(
                    "External setpoint sleep to stabilize", (0, 3000), int, "s"
                )
            )
        self._constraints = ExcitationScannerConstraints(
            ...
            idle_value_limits=(
                self._minimum_tension * self._conversion_factor,
                self._maximum_tension * self._conversion_factor,
            ),
    ...
    @transition_from(
        (
            "interrupt",
            ["prepare_scan", "prepare_step", "wait_ready", "record_scan_step"],
        )
    )
    ...
    def prepare_scan(self):
        ...
        self.log.debug(
            f"Preparing scan from {self._scan_mini} to {self._scan_maxi} with {n} points."
        )
        ncols = 4
        ...
        with self._data_lock:
            ...
            self._scan_data[:, self.frequency_column_number] = (
                np.tile(
            ...
            self._scan_data[:, self.step_number_column_number] = np.repeat(
                range(self._n_repeat), n
            )
        self._setpoint_no = 0
        if self._ext_setpoint() is not None and self._external_enabled:
            self._setpoints = np.linspace(
                start=self._setpoint_first,
                stop=self._setpoint_last,
                num=self._n_repeat // self._repeat_per_setpoint,
            )
        ...
        self._finite_sampling_io().set_frame_data(
            {
                self._output_channel: np.linspace(
                    start=self._scan_mini, stop=self._scan_maxi, num=n
                )
            }
        )
    ...
    def prepare_step(self):
        ...
        if self._repeat_no >= self._n_repeat:
            ...
            if (
                ...
                and self._ext_setpoint() is not None
                and self._repeat_no % self._repeat_per_setpoint == 0
            ):
                self.log.debug(f"Setting external setpoint.")
                self._ext_setpoint().set_setpoint(
                    self._setpoint_channel, self._setpoints[self._setpoint_no]
                )
    ...
    def wait_ready(self):
        ...
        if time.perf_counter() - self._waiting_start > self._sleep_duration:
            ...
            if self._ext_process() is not None:
                self._process_begin = self._ext_process().get_process_value(
    ...
    def record_scan_step(self):
        ...
        """
        samples_missing = (
            self._number_of_samples_per_frame * (self._repeat_no + 1)
        ...
        if samples_missing <= 0:
            ...
            if self._ext_setpoint() is not None and self._external_enabled:
                ncols += 1
                self._scan_data[offset : (offset + n), 4] = (
                    self._ext_setpoint().get_setpoint(self._setpoint_channel)
                )
                if (
                    self._repeat_no % self._repeat_per_setpoint
                    == self._repeat_per_setpoint - 1
                ):
                    self._setpoint_no = min(
                        len(self._setpoints) - 1, self._setpoint_no + 1
                    )
            if self._ext_process() is not None:
                ncols += 1
                self._process_end = self._ext_process().get_process_value(
                    self._process_channel
                )
                self._scan_data[offset : (offset + n), ncols - 1] = np.linspace(
                    start=self._process_begin, stop=self._process_end, num=n
                )
            self._scan_data[offset : (offset + n), self.time_column_number] = (
                self._step_start_time - self._scan_start_time
            ) + np.arange(n) * self._exposure_time
            self._repeat_no += 1
            self.log.debug("Step done.")
            self.watchdog_event("next")
        elif self._finite_sampling_io().samples_in_buffer < min(
            self._chunk_size, samples_missing
        ):
            pass
        else:
    ...
    def set_control(self, variable: str, value) -> None:
        ...
        if variable == "Conversion factor":
            ...
            self._scan_mini = max(
                self._minimum_tension, freq_mini / self._conversion_factor
            )
            self._scan_maxi = min(
                self._maximum_tension, freq_maxi / self._conversion_factor
            )
            self._scan_step_tension = max(
                self._minimum_tension_step, freq_step / self._conversion_factor
            )
            self._idle_value = min(
                self._maximum_tension,
                max(self._minimum_tension, idle_value / self._conversion_factor),
            )
            lims = (
                self._minimum_tension / self._conversion_factor,
                self._maximum_tension / self._conversion_factor,
            ...
            number_of_setpoints = (
                abs(self._setpoint_first - self._setpoint_last) / value
            )
    ...
    def data_format(self) -> ExcitationScanDataFormat:
        ...
        if self._ext_setpoint() is not None and self._external_enabled:
            ...
            data_column_unit.append(
                self._ext_setpoint().constraints.channel_units[self._setpoint_channel]
            )
            data_column_names.append(self._setpoint_channel)
        if self._ext_process() is not None:
            data_column_number.append(max(data_column_number) + 1)
            data_column_unit.append(
                self._ext_process().constraints.channel_units[self._process_channel]
            )
    ...
    def suggested_control_groups(self) -> OrderedDict[str, Iterable[str]]:
        ...
        result = [
            ...
            ("Time controls", ["Sleep before scan"]),
        ]
        external_controls = ["External enabled"]
        if self._ext_setpoint() is not None:
            external_controls += [
                "First external setpoint",
                "Last external setpoint",
                "External setpoint step",
                "External setpoint repeat per step",
                "External setpoint sleep to stabilize",
            ]
        result.append(("External controls", external_controls))
