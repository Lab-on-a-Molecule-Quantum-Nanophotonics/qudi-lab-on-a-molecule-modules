...
class RemoteMatisseScanner(ExcitationScannerInterface, SampledFiniteStateInterface):
    ...
    _finite_sampling_input = Connector(
        name="input", interface="FiniteSamplingInputInterface"
    )
    ...
    _stitching_first_frequency = StatusVar(
        name="stitching_first_frequency", default=0.0
    )
    ...
    _last_known_scan_parameters = StatusVar(
        name="last_known_scan_parameters", default={}
    )
    ...
    def _number_of_samples_per_frame(self):
        return round(
            (self._scan_maxi - self._scan_mini) / self._scan_speed / self._exposure_time
        )

    @property
    def _number_of_wavemeter_point_per_frame(self):
        return round(
            self._number_of_samples_per_frame
            * self._exposure_time
            * self._wavemeter().sample_rate
            * 1.5
        )
    ...
    def _init_stitch_buffer(self):
        ...
        stitch_buffer[:, self.step_number_column_number] = np.repeat(
    ...
    def prepare_stitch(self):
        no_stitch_all_done = (
            not self._stitching_enabled and self._repeat_no >= self._n_repeat
        )
        if no_stitch_all_done or (
            self._stitching_enabled
            and self._last_center_frequency >= self._stitching_last_frequency
        ):
    ...
    def go_to_position(self):
        ...
        if (
            time.perf_counter() - self._waiting_start > 1
            and not self._go_to_position_procedure_running
        ):
    ...
    def prepare_step(self):
        ...
        if self._repeat_no >= self._n_repeat:
            ...
            poly = polynomial.polyfit(
                self.scanner_positions[perm], self.sampled_frequencies[perm], deg=1
            )
        ...
        try:
            ...
            self._wavemeter().configure(
                ...
                streaming_mode=None,
                channel_buffer_size=max(
                    self._number_of_wavemeter_point_per_frame,
                    self._wavemeter().channel_buffer_size,
                ),
                sample_rate=None,
            )
            if self._finite_sampling_input().module_state() == "locked":
            ...
            if scanning_mode is not None:
                self.log.debug(
                    f"Moving to first position {scanning_mode}, scan up {self._scan_up}, delta up {delta_up}, delta down {delta_down}"
                )
    ...
    def wait_ready(self):
        ...
        if time.perf_counter() - self._waiting_start > self._sleep_time_before_scan:
            ...
            try:
                if self._scan_up:
                    self._matisse().set_setpoint(
                        "scan mode", MatisseScanMode.INCREASE_VOLTAGE_STOP_UP.value
                    )
                else:
                    self._matisse().set_setpoint(
                        "scan mode", MatisseScanMode.DECREASE_VOLTAGE_STOP_LOW.value
                    )
                self._matisse_sw().set_state("Scan Status", "RUN")
    ...
    def record_scan_step(self):
        ...
        if samples_missing <= 0:
            ...
            self.scanner_positions = np.linspace(
                start=start, stop=stop, num=len(self.sampled_frequencies)
            )
            n = self._number_of_samples_per_frame
            offset = n * self._repeat_no + self._stitch_buffer_row_index
            perm = np.argsort(self.scanner_positions)
            ...
            self.log.debug(
                f"Preparing interpolation. interp={interp.shape}, scndata {self._scan_data[offset : (offset + n), self.frequency_column_number].shape}"
            )
            self._scan_data[offset : (offset + n), self.frequency_column_number] = (
                interp
            )
            ...
            self.watchdog_event("next")
        elif self._finite_sampling_input().samples_in_buffer < min(
    ...
    def on_activate(self):
        ...
        self._constraints = ExcitationScannerConstraints(
            ...
            control_variables_list=[
                ExcitationScanControlVariable(
                    "Conversion factor", (0.0, 1e17), float, "Hz"
                ),
                ExcitationScanControlVariable(
                    "Conversion offset", (0.0, 1e17), float, "Hz"
                ),
                ExcitationScanControlVariable("Minimum scan", scan_limits, float, ""),
                ExcitationScanControlVariable("Maximum scan", scan_limits, float, ""),
                ExcitationScanControlVariable(
                    "Minimum frequency", (0.0, 1e17), float, "Hz"
                ),
                ExcitationScanControlVariable(
                    "Maximum frequency", (0.0, 1e17), float, "Hz"
                ),
                ExcitationScanControlVariable(
                    "Frequency step", (0.0, 1e17), float, "Hz"
                ),
                ExcitationScanControlVariable(
                    "Sleep before scan", (0.0, 3000.0), float, "s"
                ),
                ExcitationScanControlVariable("Idle active", (False, True), bool, ""),
                ExcitationScanControlVariable("Idle value", scan_limits, float, ""),
                ExcitationScanControlVariable(
                    "Idle frequency", scan_limits, float, "Hz"
                ),
                ExcitationScanControlVariable("Stitch scans", (False, True), bool, ""),
                ExcitationScanControlVariable(
                    "Stitching first central frequency", (0.0, 1e17), float, "Hz"
                ),
                ExcitationScanControlVariable(
                    "Stitching last central frequency", (0.0, 1e17), float, "Hz"
                ),
            ],
        )
        number_of_columns = 4 + len(self._input_channels)
        if len(self._scan_data.shape) != 2 or (
            len(self._scan_data.shape) == 2
            and self._scan_data.shape[1] != number_of_columns
        ):
        ...
        self._wavemeter().start_stream()
        time.sleep(1)
        self._conversion_offset = float(
            netobtain(self._wavemeter().read_single_point())[0][0]
        )
    ...
    def _update_conversion(self):
        ...
        self._scan_mini = max(
            0.0, (freq_mini - self._conversion_offset) / self._conversion_factor
        )
        self._scan_maxi = min(
            0.7, (freq_maxi - self._conversion_offset) / self._conversion_factor
        )
        self._idle_value = min(
            0.7,
            max(0.0, (idle_value - self._conversion_offset) / self._conversion_factor),
        )
        ...
        lims = (
            self._conversion_offset,
            0.7 * self._conversion_factor + self._conversion_offset,
        )
        self._constraints.set_limits("Minimum frequency", *lims)
        self._constraints.set_limits("Maximum frequency", *lims)
        self._constraints.set_limits(
            "Frequency step", 0, lims[1] - self._conversion_offset
        )
    ...
    def set_control(self, variable: str, value) -> None:
        ...
        if variable == "Conversion factor":
            ...
            self.set_control(
                "Minimum scan",
                (value - self._conversion_offset) / self._conversion_factor,
            )
    ...
    def data_format(self) -> ExcitationScanDataFormat:
        ...
        return ExcitationScanDataFormat(
            ...
            data_column_unit=["Hz", "", "s", ""]
            + [units[ch] for ch in self._input_channels],
            data_column_names=["Frequency", "Step number", "Time", "Stitch number"]
    ...
    def suggested_control_groups(self) -> OrderedDict[str, Iterable[str]]:
        ...
        result = [
            ...
            ("Idle controls", ["Idle active", "Idle value", "Idle frequency"]),
            (
                "Stitching controls",
                [
                    "Stitch scans",
                    "Stitching first central frequency",
                    "Stitching last central frequency",
                ],
            ),
        ]
