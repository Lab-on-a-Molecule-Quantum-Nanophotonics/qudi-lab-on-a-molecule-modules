...
class SDG6022X(PulserInterface):
    ...
    def on_activate(self):
        ...
        idn = self._inst.query("*IDN?").split(',')
        if len(idn) > 5:
            self.manufacturer = "Siglent Technologies"
            self.device_id = idn[1]
            self.hardware_version = idn[5]
        else:
            self.manufacturer = idn[1]
            self.device_id = "SDG"
            self.hardware_version = None
            self.
        self.model = idn[2]
        self.serial_number = idn[3]
        self.firmware_version = idn[4]
        self.info(f"Connected to {self.manufacturer} {self.model} {self.device_id}\nSerial: {self.serial_number}\nFirmware: {self.firmware_version}\nHardware informations: {self.hardware_version}")
        # Constraints
        ...
        self._constraints.waveform_length.max = 
    ...
    def pulser_on(self):
        ...
        """
        if self._output1_enabled:
            args = ["LOAD", self.output1load] if self.output1load is not None else []
            self.write("C1:OUTPUT ", ",".join(["ON"] + args))
        if self._output2_enabled:
            args = ["LOAD", self.output2load] if self.output2load is not None else []
            self.write("C1:OUTPUT ", ",".join(["ON"] + args))
        return 0
    ...
    def get_loaded_assets(self):
        ...
        if 1 in channel_numbers:
            response = self.query("C1:ARWV?").split(",")[-1]
            loaded_assets[1] = response
        if 2 in channel_numbers:
            response = self.query("C2:ARWV?").split(",")[-1]
    ...
    def get_status(self):
        ...
        status1 = "ON" in self.query("C1:OUTPUT?")
        status2 = "ON" in self.query("C2:OUTPUT?")
    ...
    def get_sample_rate(self):
        ...
        """
        return float(self.query("C1:SAMPLERATE?").split(",")[-1])
    ...
    def get_active_channels(self, ch=None):
        ...
        if output1_asked:
            active_ch["a_ch1"] = "ON" in self.query("C1:OUTPUT?")
        if output2_asked:
            active_ch["a_ch2"] = "ON" in self.query("C2:OUTPUT?")
        return active_ch
    ...
    def set_active_channels(self, ch=None):
        ...
        if not set(currently_active_channels).issuperset(ch):
            self.log.error("Trying to (de)activate channels that are not present in the AWG.\n"
                           "Operation aborted.")
            return currently_active_channels
        if "a_ch1" in ch and ch["a_ch1"] is not currently_active_channels["a_ch1"]:
            st = "ON" if ch["a_ch1"] else "OFF"
            args = ["LOAD", self.output1load] if self.output1load is not None and ch["a_ch1"] else []
            args = ",".join(args)
            self.write(f"C1:OUTPUT {st},{args}")
        if "a_ch2" in ch and ch["a_ch2"] is not currently_active_channels["a_ch2"]:
            st = "ON" if ch["a_ch2"] else "OFF"
            args = ["LOAD", self.output2load] if self.output2load is not None and ch["a_ch2"] else []
            args = ",".join(args)
            self.write(f"C2:OUTPUT {st},{args}")
    ...
    def write_waveform(self, name, analog_samples, digital_samples, is_first_chunk, is_last_chunk,
        ...
        if len(analog_samples) == 0:
            ...
            return -1, waveforms
        if total_number_of_samples > self._constrains.waveform_length.max:
            self.log.error(f"Unable to write {total_number_of_samples} samples. The maximum allowed number of samples is {self._constrains.waveform_length.max}.")
            return -1, waveforms
        if total_number_of_samples < self._constrains.waveform_length.min:
            self.log.error(f"Unable to write {total_number_of_samples} samples. The minimum allowed number of samples is {self._constrains.waveform_length.min}.")
            return -1, waveforms
        ...
        for a_ch in analog_samples.keys():
            ...
            if this_length > total_number_of_samples:
                ...
                self.log.error(f"The anaolg samples for channel {a_ch} are different from the chunk length ({this_length} instead of {chunk_length}).")
        for a_ch in analog_samples.keys():
            if a_ch == "a_ch1":
                chosen_channel = 1 
            elif a_ch == "a_ch2":
            ...
            if np.min(s) < -1 or np.max(s) > 1:
                s = np.clip(s, -1, 1)
                self.log.warning(f"Got waveform out of the range [-1,1]. Output got clipped.")
            mini = np.iinfo(np.int16).min
            maxi = np.iinfo(np.int16).max
            s = np.astype((s/2 + 0.5) * (maxi - mini) + mini, np.int16)
            filename = f'{name}_ch{chosen_channel}'
            filepath = os.path.join(self._tmp_work_dir, filepath + '.bin')
            with open(filepath, 'ab') as binfile:
                s.tofile(binfile)
            if is_last_chunk and self._inst is not None:
                s = np.fromfile(filepath, dtype=np.int16)
                self._inst.write_binary_values(f"C{chosen_channel}:WVDT,LENGTH,{total_number_of_samples},WVNM,{filename},WAVEDATA,", s)
    ...
    def get_waveform_names(self):
        ...
        """
        return self._inst.query("STL? USER").split(",")[1:]
